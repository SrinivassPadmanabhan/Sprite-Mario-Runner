...
var indexedDbUtil = (function(){
    ...
    IDBCreation.prototype.getAll = function(objectStoreName){
        var resultArr = [],
            transaction = db.transaction(objectStoreName, TransactionType.READ_ONLY),
            objectStore = transaction.objectStore(objectStoreName),
            req = objectStore.openCursor();
        req[TransactionState.success] = function(event){
            var cursor = event.target.result;
            if(cursor){
                resultArr.push(cursor.value);
            } else {
                this[TransactionState.success] && this[TransactionState.success](resultArr);
            }
        }
        req[TransactionState.error] = function(event){
            this[TransactionState.error] && this[TransactionState.error](event.target);
        }
    }
    function upgradeNeeded(){ //updating or creating the ObjectStore and Index in the database...
        ...
        for(var i = 0, n = this.tableDetails.length; i < n; i++){   
            ...
            objectStore.transaction[TransactionState.complete] = function(_event) {
                trace("Object Store is Created for all successfully ===>");
                
            };
            objectStore.transaction[TransactionState.error] = function(_event) {
